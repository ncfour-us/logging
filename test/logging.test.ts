...
test("logger-prefixes", () => {
  ...
  expect(logSpy.mock.calls[0][0]).toMatch(
    /^\[ERR\] \(test3\): test ERROR message 1$/,
  );
  expect(logSpy.mock.calls[1][0]).toMatch(
    /^\[INF\] \(test3\): test INFO message 1$/,
  );
  expect(logSpy.mock.calls[2][0]).toMatch(
    /^\[INF\] \(test3\): test INFO message 2$/,
  );
...
test("logger-timestamps", () => {
  ...
  expect(logger).toBeDefined();
  expect(logSpy.mock.calls[0][0]).toMatch(
    /^\[[0-9-:.TZ]*\] \(test4\): test ERROR message 1$/,
  );
  expect(logSpy.mock.calls[1][0]).toMatch(
    /^\[[0-9-:.TZ]*\] \(test4\): test INFO message 1$/,
  );
  expect(logSpy.mock.calls[2][0]).toMatch(
    /^\[[0-9-:.TZ]*\] \(test4\): test INFO message 2$/,
  );
